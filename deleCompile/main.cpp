...
int main(int argc, char *argv[])
{
    ...
    for(i=0; i<fNum; i++) fileList << codec1->toUnicode(argv[i+2]);

    FILE *tFile;
    FILE *resFile;
/*
    resFile = fopen("res.405", "wb");
...
*/
    int  First_Pass = TRUE;
      ...
      /*     }
        ...
        for(fi=0; fi<fNum; fi++)
        {
            inFile2 = fopen(fileList.at(fi).toAscii().constData(),"r");
            if (inFile2==NULL)
            {
                Warning(3);
                return 3;
