...
//append environment
    ...
    for(i=0;i<mNum;i++)
    {
        ...
        oc_rec->obsCode = mpc_rec->getObsCode();
        eqFile.ocList << oc_rec;

        isObj = 1;

        for(j=0; j<envSize; j++)
        {
            if(QString().compare(objName, pList[j]->name)==0)
            {
                isObj=0;
                break;
            }
