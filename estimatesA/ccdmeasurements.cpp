...
double syscorr_ra(double mag)
{//1|104.220665518119|3|4|12.7734525025536|2148.27423307453|-360.303617575313|15.1720016802259|-37118.0654343914|7723.33968474359|-528.542490362857|11.8626714601832|
	double res;
	if (mag<=12.7734525025536) res = 2148.27423307453 - 360.303617575313*mag + 15.1720016802259*pow(mag,2);
						else   res = -37118.0654343914 + 7723.33968474359*mag -528.542490362857*pow(mag,2)+11.8626714601832*pow(mag,3);
	return res;	
};

double syscorr_de(double mag)
{
	double res = -17674.5950566468+5640.16643975834*mag-665.099897042149*pow(mag,2)+34.0934997438143*pow(mag,3)-0.636135808019977*pow(mag,4);
	return res;
};

double syscorr_mag(double mag)
{
	double res = -22.200387327086 - 0.210235299019587*mag + 0.761478141722307*pow(mag,2) - 0.0693486196619695*pow(mag,3) + 0.00172140573785212*pow(mag,4); 
	return res;
};
