...
void sscatbin::s2rec(char *str_in)
{
	char *tstr;
...
void sscatbin::rec2s(char *str_out)
{
	char *tstr;
	tstr = new char[SIMPLE_STR];
	...
	sprintf(tstr, "%s ", this->record->LSPM);
...
int sscatbin::GetRecNameLSPM(char *oname)
{
	strcpy(&oname[11], "\0");
	for(int i=0; i<this->nstr; i++)
	{
		this->GetRec(i);
		if(streqv(this->record->LSPM, oname)) return i;
...
/*
int sscatbin::GetRecName(char *name);
int sscatbin::GetRecNum(int num);
int sscatbin::AddRec(int pos);
int sscatbin::AddRec();
*/
