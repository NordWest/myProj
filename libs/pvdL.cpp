...
int pvd::initDT()
{
	...
	if(this->klog)
	{
		...
		strMV = new char[1000];
/*
		this->dtr->f->print(strMV);
		fprintf(this->FLOG, "dtr\n%s\n", strMV);
		printf("dtr\n%s\n", strMV);
		this->dtd->f->print(strMV);
		fprintf(this->FLOG, "dtd\n%s\n", strMV);
		printf("dtd\n%s\n", strMV);
//
		this->dtr->x->print(strMV);
		fprintf(this->slog->FLOG, "xdtr\n%s\n", strMV);
		printf("xdtr\n%s\n", strMV);
		this->dtd->x->print(strMV);
		fprintf(this->slog->FLOG, "xdtd\n%s\n", strMV);
		printf("xdtd\n%s\n", strMV);

		for(i=0; i<this->nt; i++)
		{
			printf("%f ", this->dtr->f->get(i, 0));
		}
*/
		delete(strMV);
	}
...
*/
	double d;
	this->t = 0.0;
	for(int i=0; i<this->obs->nmpcs; i++)
	{
		this->t += this->dtr->x->get(i);
	}
	this->t /= this->obs->nmpcs;
...
//	printf("\n%s\n", str);
	...
	fprintf(this->FC, "\nm	%f\npsi	%f\nmt	%f\nc	%f\n", this->m->val*206265.0, this->psi->val*180.0/PI, this->mt->val*206265.0, this->c->val);
