...
CPOS getCposFromWCS(double x, double y, double *WCSD)
{
   ...
   vector<double> posC = getTangToRaDe(ksi*3600, eta*3600, WCSD[2]*3600000, WCSD[3]*3600000);
   //pos.ra = WCSD[2] + (x - WCSD[0])*WCSD[8] + (y - WCSD[1])*WCSD[9];
   //pos.de = WCSD[3] + (x - WCSD[0])*WCSD[10] + (y - WCSD[1])*WCSD[11];
   //pos.ra = pos.ra*3600000;pos.de = pos.de*3600000;
   pos.ra = posC(0);
   pos.de = posC(1);
   return pos;
};
